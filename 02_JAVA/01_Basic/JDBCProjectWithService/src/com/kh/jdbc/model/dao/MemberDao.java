...
public class MemberDao {
	...
	public int bulkInsertMembers(Member m, Connection conn) {
			int result = 0;
			PreparedStatement pstmt = null;
				
			String sql = prop.getProperty("bulkInsertMembers");
	...
	public int bulkDeleteMembers(String[] userIds, Connection conn) {
	    ...
	    for(int i=0; i<userIds.length; i++) {
	        sb.append("?");
	        if(i < userIds.length - 1) sb.append(",");
	    }
	    ...
	    try {
	        ...
	        for(int i=0; i<userIds.length; i++) {
	            pstmt.setString(i+1, userIds[i].trim());
	        }
