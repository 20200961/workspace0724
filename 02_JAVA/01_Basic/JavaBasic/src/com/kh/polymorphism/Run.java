...
//		// Cake까지 접근 가능하며 메모리 공간은 실제 CheeseCake의 공간까지 존재
//		c1.sweet();
//		// 오버리딩시에는 실제 메모리에 있는 재정의된 메서드가 호출됨
//		c1.yummy();
//		// 다운 캐스팅 : 자식의 참조변수로 타입을 변경(업캐스팅관계에 있을 경우에만 가능)
//		((CheeseCake)c1).milky();
//		
		...
		// 1. 부모 타입의 래퍼런스(참조변수)로 부모객체를 다루는 경우
		Car c1 = new Car("빨간색", "가솔린", 2021);
		c1.drive();
		// ((Sonata)c1).moveSonata();
		
		// 2. 자식 타입 래퍼런스 자식 객체를 다루는 경우
		Avante c2 = new Avante("흰색","LPG",2022);
		c2.drive();
		c2.moveAvante();
		// 부모참조변수로 업캐스팅 가능 -> 오버라이딩된 메서드는 생성된 메모리 기준으로 호출됨
		((Car)c2).drive();
		// ((Car)c2).moveAvante(); 업 캐스팅시에는 자식메모리 접근 불가
