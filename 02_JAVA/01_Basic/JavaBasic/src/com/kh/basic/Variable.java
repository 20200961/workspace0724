...
public class Variable {
	...
	public static void main(String[] args) {
		...
		// 실수형(소수점 6자리까지 사용) 자료형으로 num2라는 이름의 변수 선언
		float num2;
		num2 = 3.14123546431f;
		 ...
		 * 
		 * 정수형 리터럴의 기본값은 int 
		 * byte, short은 범위내에 있을 경우 자동형 변환된다.
		 */
		int in = 10;
		byte by = 10;
		short sh = 10;
		 ...
		 * 문자열
		 * String
		 * 문자열 같은 경우 할당할 메모리 크기가 가변적이므로 원시타입이 아닌 객체 타입으로 변수를 만들어 사용함
		 * 
		 * 원래 객체를 만드는 방식
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열 내에서 탭, 백슬러시, 작따옴표 등을 사용하기 위한 방식
		 */
		 ...
		 * 파스칼 케이스 : 카멜케이스에서 첫글자도 대문자로 작성(자바에서 클래스명에 사용)
		 * ex) UserName, TotalPrice
		 * 
		 ...
		 * ex) USER_NAME
		 * 
		 * 케밥케이스 : 단어를 소문자로 작성하되 문자와 문자사이를 하이폰(-)으로 이어주는 것(html 속성, url, 설정파일들)
		 * ex) user-name
		 */
