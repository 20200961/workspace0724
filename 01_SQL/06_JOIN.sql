...
-- JOIN절에 같이 조인하고자 하는 테이블을 기술 + 매칭 시킬 컬럼에 대한 조건
-- [INNER] JOIN [USING/ON]

-- 1. 연결할 두 컬럼명이 다른 경우 JOIN ON을 사용해서 연결할 컬럼명을 명시적으로 연결
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID AND EMP_ID = 201);
...
SELECT E.EMP_ID, E.EMP_NAME, E.BONUS FROM EMPLOYEE E
...
SELECT E.EMP_ID, E.EMP_NAME, E.BONUS, D.DEPT_TITLE FROM EMPLOYEE E
JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID
