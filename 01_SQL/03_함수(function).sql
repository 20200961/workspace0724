...
-- 앞에서부터 다른 문자가 나올 때 까지 공백을 제거
select LTRIM('    K  H   ') from DUAL;

select LTRIM('123123KH123', '123') from DUAL;    

select RTRIM('123123KH123', '123') from DUAL;    
-- 제거하고자 하는 문자는 문자열이 아닌 문자들
...
-- 기본 차수는 소수점 첫번째 자리에서 반올림
select round(123.456) from DUAL; 
-- 위치값이 양수로 증가할수록 소수점 뒤로 한칸씩 이동한다.
select round(123.456,1) from DUAL;
-- 위치값이 음수로 감소할수록 소수점 앞으로 한칸씩 이동한다.
select round(123.456,-1) from DUAL;
...
-- 사원 테이블에서 사원명, 입사일, 수습시간 종료일(입사일로부터 3개월 뒤) 조회
...
SELECT TO_DATE('2025', 'YYYY'),
       ...
       TO_DATE('25',   'YYYY'),
       TO_DATE('25',   'RRRR')
       TO_DATE('60', 'YY'),  -- YY: 항상 입력 그대로 해석 → 현재 세기를 반영
...
/* 
    TO_DATE : 숫자 타입 또는 문자타입을 날짜타입으로 변경하는 함수
    
    TO_DATE(숫자 | 문자, [포맷]) -> DATE
*/

select to_date(20100101) from dual;
-- 기본적으로 연도를 2자리만 입력시 RR룰이 적용된다.
...
/* 
    CASE문
    SQL에서 조건에 따라 값을 분리하고 싶을때 사용하는 구문
