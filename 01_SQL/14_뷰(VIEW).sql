/*
    ...
    자주사용하는 SELECT문을 하나의 가상 테이블처럼 지정해 놓은 객체
    실제 데이터를 저장하지 않고, 정의된 SELECT문을 실행해서 데이터를 조회
    복잡한 조인쿼리를 매번 작성하지 않고 간단하게 사용할 수 있음
...
-- GRANT CREATE VIEW TO C##SERVER; 뷰 생성권한
