...
--7. NONO EMPLOYEE 테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급, 고용일, 연락처 조회
...
select EMP_NAME,SALARY,(SALARY+SALARY*NVL(BONUS,0))*12 - SALARY*12*0.03 실수령액 , HIRE_DATE FROM EMPLOYEE
...
WHERE DEPT_CODE IN ('D9','D5') AND HIRE_DATE < TO_DATE('02/01/01','RR/MM/DD');
--11.EMPLOYEE 테이블에 고용일이 90/01/01 ~ 01/01/01인 사원의 전체 내용 조회
SELECT * FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN TO_DATE('90/01/01','RR/MM/DD') AND TO_DATE('01/01/01','RR/MM/DD')
...
SELECT * FROM EMPLOYEE
WHERE DEPT_CODE IN ('D9','D6') AND HIRE_DATE BETWEEN TO_DATE('90/01/01','RR/MM/DD') 
AND TO_DATE('00/12/01','RR/MM/DD') AND SALARY >= 2700000
AND LENGTH(SUBSTR(EMAIL, 1, INSTR(EMAIL, '_') - 1)) = 4;
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO,1,2) || '년' || SUBSTR(EMP_NO,3,2) || '월' || SUBSTR(EMP_NO,5,2) || '일' AS "생일",
...
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6), SUBSTR(EMP_NO,8,7), SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,7) FROM EMPLOYEE
WHERE EMP_ID = 201;
